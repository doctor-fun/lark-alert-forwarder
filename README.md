...
也兼容只会填 URL、不能加请求头的来源（例如 DataWorks）：走 `POST /dataworks/alert`，用查询参数带 token 和路由信息。
...
```bash
docker build -t matrix-alert-forwarder .
...
- Oncall file example: `configs/alert-oncall.example.yaml`
